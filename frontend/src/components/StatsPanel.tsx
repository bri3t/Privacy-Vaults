...
export function StatsPanel({ selectedVault, networkConfig }: StatsPanelProps) {
  ...
  return (
    <div className="glass-card rounded-2xl shadow-xl shadow-violet-500/5 p-5 flex flex-col h-full">
      ...
      {/* Anonymity set */}
      <div className="mb-4">
        <p className="text-xs text-zinc-400 mb-0.5">Anonymity set</p>
        {isLoading ? (
          ...
          <p className="text-white">
            ...
            <span className="text-xs text-zinc-400 ml-2">equal user deposits</span>
          </p>
        )}
      </div>
      ...
      <div>
        <p className="text-xs text-zinc-400 mb-2">Latest deposits</p>
        {isLoading ? (
          <div className="flex items-center gap-2">
            <span className="inline-block w-3 h-3 rounded-full border-2 border-violet-400 border-t-transparent animate-spin" />
          ...
          <div className="grid grid-cols-2 grid-rows-5 gap-x-4 gap-y-1">
            {Array.from({ length: 10 }, (_, i) => {
              const d = latest10[i]
              return d ? (
                <div key={d.leafIndex} className="py-1 px-2 rounded bg-zinc-800/40">
                  <span className="text-xs font-mono text-zinc-400">{d.leafIndex}.</span>{' '}
